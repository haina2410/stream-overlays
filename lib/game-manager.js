...
export function createGameManager({ registry, file, logger = console }) {
  ...
  for (const game of definitions) {
    const store = game.createStore({ initialState: saved.games[game.id] });
    if (!store || typeof store !== 'object') throw new Error(`${game.id}.store must be an object`);
    for (const field of ['get', 'onChange']) {
      if (typeof store[field] !== 'function') throw new Error(`${game.id}.store.${field} must be a function`);
    }
    stores.set(game.id, store);
  }
